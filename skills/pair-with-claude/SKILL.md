...
Treat Claude as a second teammate, not an authority. Ask it to inspect evidence, challenge assumptions, and disagree candidly. You are the coordinator, final decision-maker, active-checkout writer, verifier, and sole integrator.
...
7. For every foreground non-interactive turn, use `claude -p --output-format json` and capture the terminal `result` object. Use `claude -p --output-format stream-json --verbose` instead only when per-step telemetry is required. If runtime discovery shows that these flags are unavailable or renamed, use the discovered machine-readable equivalent and report the downgrade.
...
- Keep one writer per root. Claude may inspect the active checkout, but you remain the active checkout's only writer unless the user explicitly authorizes a serialized handoff. Never let parallel agents write the same checkout.
...
- Give Claude direct shell execution only when verified OS enforcement fails closed around files, processes, credentials, environment, and network. Otherwise keep shell unavailable and run reviewed commands yourself.
...
Freeze relevant files during each Claude turn. Ask for an independent first pass before revealing your preferred conclusion when anchoring could weaken the review. Let Claude request diagnostics, tests, benchmarks, or experiments; execute the exact reviewed operation yourself and return redacted output, exit status, and resulting state.
...
Independently verify every actionable claim and keep a disposition ledger: `accepted`, `rejected_with_evidence`, `needs_experiment`, or `deferred_by_user`. You remain the writer. Fix accepted findings and run proportionate checks; never change the candidate merely to satisfy Claude.
...
Grant only file inspection and edit capabilities by default. Protect Git metadata, repository instructions, hooks, validation entrypoints, credentials, and every out-of-scope path. Broker commands yourself unless the discovered OS sandbox meets the common boundary.
...
Always include a `Pairing telemetry` section in the final user-facing report, even when Claude failed, did not complete, was unavailable, or emitted no telemetry. For every attempted invocation, report its status, actual model identity, actual reasoning-effort setting, and usage. If Claude was not invoked, report `not run` and the reason. Never omit the section.

Prefer the terminal result's `modelUsage` and structured initialization or session metadata for observed model identities. Derive actual effort from emitted runtime metadata, not merely from an accepted `--effort` argument. Show the requested value when it differs from the observed value or when the actual value is unverified. Label an unobservable actual model or effort `unverified`; never present requested routing as actual. List fallback, helper, or subagent models separately when more than one model ran.

Capture Claude's reported usage for every completed invocation when available. Prefer the terminal result's `modelUsage` for whole-tree token and cost accounting when present because its top-level `usage` can exclude nested agent work. Include the fields actually returned in the final user-facing report, such as input, cache-creation, cache-read, and output tokens, estimated cost, and duration, and label them as invocation-scoped. Do not infer a separate reasoning or thinking count from output tokens unless the CLI emits one explicitly.

When a result object or individual field is missing, report it as `unavailable` with a brief reason. When the task resumes the same Claude session across multiple CLI invocations, retain each invocation's result and aggregate them manually for a complete session report. Present the session aggregate separately from the latest invocation, preserve field semantics while summing, and identify any invocation excluded from the aggregate because its usage is unavailable.
